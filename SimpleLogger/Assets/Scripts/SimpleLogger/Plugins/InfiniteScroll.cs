...
namespace Mopsicus.InfiniteScroll
{
    ...
    public class InfiniteScroll : MonoBehaviour, IDropHandler
    {
        ...
        /// <summary>
        /// Callback before the item is recycled
        /// </summary>
        public Action<int, GameObject> OnRecycle = delegate { };
        ...
        /// <summary>
        /// Width position buffer to prevent changing of data when gameobject is visible
        /// </summary>
        public float _widthOffset;

        /// <summary>
        /// Height position buffer to prevent changing of data when gameobject is visible
        /// </summary>
        ...
        void Update()
        {
            if (Type == 0)
            {
                UpdateVertical();
                if (scrollbar != null)
                    scrollbar.value = _scroll.verticalNormalizedPosition;
            }
            else
            {
                UpdateHorizontal();
                if (scrollbar != null)
                    scrollbar.value = _scroll.horizontalNormalizedPosition;
            }
        }
        ...
        void UpdateVertical()
        {
            ...
            float _topPosition = _content.anchoredPosition.y - ItemSpacing + _heightOffset;
            ...
            if (position < 0 || _previousPosition == position ||
                (!IsForceUpdate && _scroll.velocity.y == 0f))
            {
                return;
            }
            IsForceUpdate = false;
        ...
        void UpdateHorizontal()
        {
            ...
            float _leftPosition = _content.anchoredPosition.x * -1f - ItemSpacing - _widthOffset;
        ...
        float CalcSizesPositionsHorizontal(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                _widths[i] = OnWidth?.Invoke(i) ?? 100;
        ...
        void CreateViewsVertical()
        {
            ...
            int heightCount = _heights.Count > 0 ? _heights.Count : 1;
            height = height / heightCount;
            height = height > 0 ? height : 1;
            int fillCount = Mathf.RoundToInt(_container.height / height) + 10;
        ...
        void CreateViewsHorizontal()
        {
            ...
            int fillCount = Mathf.RoundToInt(_container.width / width) + 8;
            _views = new GameObject[fillCount];
            for (int i = 0; i < fillCount; i++)
            {
                clone = (GameObject)Instantiate(Prefab, Vector3.zero, Quaternion.identity);
